...
# --- combined report across several scans (used by the monitor) --------------
_HTML_COLORS = {"high": "#ea580c", "medium": "#ca8a04", "low": "#2563eb", "info": "#6b7280"}


def _sev_totals(results) -> dict:
    tot = {"high": 0, "medium": 0, "low": 0, "info": 0}
    for r in results:
        for s, n in r.by_severity.items():
            tot[s] = tot.get(s, 0) + n
    return tot


def _sev_cards_html(counts: dict) -> str:
    return "".join(
        f'<div class="card" style="border-top:4px solid {_HTML_COLORS[s]}">'
        f'<div class="n" style="color:{_HTML_COLORS[s]}">{counts.get(s, 0)}</div>'
        f'<div class="cap">{s}</div></div>'
        for s in ("high", "medium", "low", "info"))


def _finding_cards_html(findings, esc) -> str:
    if not findings:
        return '<p class="ok">&#10003; No issues found &mdash; nice.</p>'
    return "\n".join(
        f'<details class="finding" style="border-left-color:{_HTML_COLORS.get(f.severity, "#6b7280")}">'
        f'<summary><span class="fh">{i}. {esc(f.title)}</span>'
        f'<span class="sev" style="background:{_HTML_COLORS.get(f.severity, "#6b7280")}">{esc(f.severity)}</span></summary>'
        f'<div class="fbody"><div class="kv"><b>Evidence</b></div><pre>{esc(f.evidence)}</pre>'
        f'<div class="kv"><b>Remediation</b></div><div class="rem">{esc(f.remediation)}</div></div></details>'
        for i, f in enumerate(findings, 1))


def to_html_multi(results, title: str = "Monitor Report") -> str:
    """One collapsible HTML page across several scans — a summary table + per-target findings."""
    esc = _html.escape
    gen = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    logo = _logo_uri()
    nav = _HTML_NAV.replace("__LOGO__", logo)
    total = _sev_totals(results)

    banner = (f'<div class="banner"><div class="bhead">'
              f'<img class="blogo" src="{logo}" alt="Temple Guard">'
              f'<div><div class="brand">Temple Guard &middot; Monitor Report</div>'
              f'<h1>{esc(title)}</h1>'
              f'<div class="bmeta">{len(results)} scans &middot; {gen}</div></div></div></div>')
    head = ('<!doctype html><html lang="en"><head><meta charset="utf-8">'
            '<meta name="viewport" content="width=device-width,initial-scale=1">'
            '<title>temple-guard - monitor report</title><style>' + _HTML_CSS +
            'table.tgt{width:100%;border-collapse:collapse;margin:14px 0}'
            'table.tgt th,table.tgt td{border-bottom:1px solid #e5e7eb;padding:7px 10px;text-align:left;font-size:13px}'
            'table.tgt td.n{text-align:right;font-weight:700}'
            'table.tgt th{color:#6b7280;text-transform:uppercase;font-size:11px}</style></head><body>')

    rows = "".join(
        f'<tr><td><code>{esc(r.url)}</code></td>'
        f'<td>{"reachable" if r.reachable else "<span style=color:#ea580c>unreachable</span>"}</td>'
        + "".join(f'<td class="n" style="color:{_HTML_COLORS[s]}">{r.by_severity.get(s, 0)}</td>'
                  for s in ("high", "medium", "low", "info")) + "</tr>"
        for r in results)
    table = ('<table class="tgt"><thead><tr><th>Target</th><th>Status</th>'
             '<th>High</th><th>Med</th><th>Low</th><th>Info</th></tr></thead><tbody>'
             + rows + "</tbody></table>")

    sections = []
    for r in results:
        sections.append(f'<h2>{esc(r.url)}</h2>')
        if not r.reachable:
            sections.append(f'<p class="muted">Could not reach the target.</p><pre>{esc(r.error)}</pre>')
        else:
            sections.append(f'<div class="summary">{_sev_cards_html(r.by_severity)}</div>'
                            + _finding_cards_html(r.findings, esc))

    return (head + nav + banner +
            '<div class="page"><h2>Summary</h2>'
            f'<p class="muted">{len(results)} scans &middot; {sum(total.values())} findings '
            f'({total["high"]} high · {total["medium"]} medium · {total["low"]} low · {total["info"]} info)</p>'
            f'<div class="summary">{_sev_cards_html(total)}</div>{table}'
            '<h2>Per-target findings</h2>' + "".join(sections) +
            '<p class="foot">Generated by temple-guard &mdash; run only against apps you own or are authorized to test.</p>'
            '</div></body></html>')


def to_markdown_multi(results, title: str = "temple-guard — monitor report") -> str:
    """Combined Markdown across several scans — a summary table + per-target findings."""
    gen = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    total = _sev_totals(results)
    emoji = {"high": "🔴", "medium": "🟠", "low": "🔵", "info": "⚪"}
    lines = [f"# {title}", "",
             f"_{len(results)} scans · {sum(total.values())} findings "
             f"({emoji['high']} {total['high']} · {emoji['medium']} {total['medium']} · "
             f"{emoji['low']} {total['low']} · {emoji['info']} {total['info']}) · {gen}_", "",
             "| Target | Status | High | Med | Low | Info |", "|---|---|---:|---:|---:|---:|"]
    for r in results:
        c = r.by_severity
        lines.append(f"| `{r.url}` | {'reachable' if r.reachable else 'unreachable'} | "
                     f"{c['high']} | {c['medium']} | {c['low']} | {c['info']} |")
    lines += ["", "---", ""]
    for r in results:
        lines.append(f"## {r.url}")
        if not r.reachable:
            lines += [f"> unreachable — {r.error}", ""]
            continue
        if not r.findings:
            lines += ["_No issues found._", ""]
            continue
        for f in r.findings:
            lines.append(f"- {emoji.get(f.severity, '')} **{f.title}** — {f.remediation}")
        lines.append("")
    lines.append("_Generated by temple-guard._")
    return "\n".join(lines)
